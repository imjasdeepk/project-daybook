...
def test_yearly_birthday_is_found_a_year_out(paths, run):
    run("event", "add", "--summary", "Dad birthday", "--date", "14 March 1980")
    found = events.on_date(paths.events, date(2027, 3, 14))["events"]
    assert len(found) == 1
    assert found[0]["years_since_first"] == 47


def test_upcoming_window_crosses_the_year_boundary(paths, run):
    run("event", "add", "--summary", "January date", "--date", "1980-01-05")
    run("event", "add", "--summary", "December date", "--date", "1980-12-20")
